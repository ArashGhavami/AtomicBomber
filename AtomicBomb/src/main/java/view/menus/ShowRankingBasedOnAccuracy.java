...
public class ShowRankingBasedOnAccuracy extends Application {
    ...
    public double getUserKill(User user){
        ...
        for(GameLauncher gameLauncher : user.getGameLaunchers()){
            averageAccuracy += ((double) gameLauncher.getShotsInTarget() /gameLauncher.getShotsUsed()) * 100;
        }
        averageAccuracy = averageAccuracy/user.getGameLaunchers().size();
    ...
    public void printColorFull(ArrayList<User> users, VBox vBox) {
        ...
        if(users.size() > 3){
            for(int i =0 ; i<3; i++){
                ...
                label.setText(users.get(i).getUsername() + " + " + String.format("%.2f", getUserKill(users.get(i))));
